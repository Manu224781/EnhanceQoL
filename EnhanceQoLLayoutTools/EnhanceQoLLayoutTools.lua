...
local function addLayoutToolsToTree()
    addon.functions.addToTree("ui", { value = "move", text = L["Move"] })
end
...
local function addGeneralSettings(container)
    ...
    local cbMove = addon.functions.createCheckboxAce(L["Global Move Enabled"] or "Global Move Enabled", db["uiScalerGlobalMoveEnabled"], function(_, _, val)
        db["uiScalerGlobalMoveEnabled"] = val
    end)
    groupGlobal:AddChild(cbMove)

    local cbScale = addon.functions.createCheckboxAce(L["Global Scale Enabled"] or "Global Scale Enabled", db["uiScalerGlobalScaleEnabled"], function(_, _, val)
        db["uiScalerGlobalScaleEnabled"] = val
    end)
    groupGlobal:AddChild(cbScale)

    local cbMoveMod = addon.functions.createCheckboxAce(L["Require Modifier For Move"] or "Require modifier to move", db["uiScalerMoveRequireModifier"], function(_, _, val)
        db["uiScalerMoveRequireModifier"] = val
    end)
    ...
    for _, entry in ipairs(addon.LayoutTools.variables.knownFrames or {}) do
        ...
        local cb = addon.functions.createCheckboxAce(label, val, function(_, _, v)
            ...
            if entry.names then
                for _, n in ipairs(entry.names) do hookName(n) end
            end
        end)
        -- two columns
...
local function addGenericFrameOptions(container, frameName, displayText)
    ...
    -- Enable scaling
    local cbScale = addon.functions.createCheckboxAce(L["Enable scaling for"] and (L["Enable scaling for"] .. ": " .. (displayText or frameName)) or ("Enable scaling: " .. (displayText or frameName)), db[keys.enable], function(_, _, val)
        db[keys.enable] = val
    ...
    -- Enable moving
    local cbMove = addon.functions.createCheckboxAce(L["Enable moving for"] and (L["Enable moving for"] .. ": " .. (displayText or frameName)) or ("Enable moving: " .. (displayText or frameName)), db[keys.move], function(_, _, val)
        db[keys.move] = val
        if frame and frame:IsShown() then
            -- ensure hooks exist
...
local function tryHookKnownFrames()
    for _, entry in ipairs(addon.LayoutTools.variables.knownFrames or {}) do
        ...
        if entry.addon and not (IsAddonLoaded and IsAddonLoaded(entry.addon)) then
            shouldHook = false
        end
        if shouldHook then
            if entry.names then
...
local eventHandlers = {
    ["ADDON_LOADED"] = function(arg1)
        if arg1 == addonName then
            ...
            tryHookKnownFrames()
            if addon.LayoutTools and addon.LayoutTools.functions and addon.LayoutTools.functions.ensureWheelCaptureOverlay then
                addon.LayoutTools.functions.ensureWheelCaptureOverlay()
            end
        end
